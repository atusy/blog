...
categories: [stats]
tags: [stats, missing value, R, ggplot2]
draft: yes
---
...
- MAR: xの欠損がyの値に依存する場合
    - y > 0 ならば欠損しうる  
      (正しくは1000点のy中、yの値が上位500位以内の場合)
- MNAR: xの欠損がxの値に依存する場合
    - x > 0 ならば欠損しうる
      (正しくは1000点のx中、xの値が上位500位以内の場合)
...
当記事で作成した図のソースコードは[こちら](https://github.com/atusy/blog/tree/master/content/post/2018-08-24/missing-value-type.Rmd)。
