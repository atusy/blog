...
# packed data frame を良い感じに表示用の表にする

Display table (表示用の表) は `gt` パッケージが定義している用語で，R 内部で扱うための表 (data frame) に対して，人に見せるための表くらいの認識でいいと思う．
...
## 通常の data frame で列の階層性を表現した Display table を作る
...
## packed data frame で列の階層性を表現した Display table を作る
