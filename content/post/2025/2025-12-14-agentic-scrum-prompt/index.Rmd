...
初めてのスプリントでは、Retrospectiveの対象となる前回スプリントが存在しない。そのため、Sprint 0は以下の流れで開始する:

```
Refinement -> Planning -> Execution -> Review -> (Sprint 1 の Retro)
```

**Sprint 0の準備事項:**

- Product Goalが定義されている
- Product Backlogに少なくとも1つのPBIが存在する（`draft`でも可）
- Definition of Doneが設定されている
- `scrum.yaml`が初期化されている
