...
- すべてのアーティファクトは単一ファイル（`scrum.ts`）に集約
...
| 原則 | 説明 |
|------|------|
| Single Source of Truth | すべてのアーティファクトは`scrum.ts`に集約する |
...
```
Refinement -> Planning -> Execution -> Review -> Retro -> scrum.ts compaction -> (Next Refinement)
```
...
| イベント | 目的 | 完了条件 |
|----------|------|----------|
| Retrospective | 前回の改善アクションの効果検査、新たな改善の特定 | immediateアクションが完了、他は文書化されている |
...
| Compaction | ダッシュボードの剪定 | 行数が300行以下 |
...
| 原則 | AIエージェント向け解釈 |
|------|------------------------|
| Independent | 他PBIから独立して優先順位を変更できる、かつ人間への依存がない |
| Negotiable | 成果を明確にしつつ実装手段は柔軟に |
| Valuable | User Story形式でユーザー価値を明示 |
...
- ValuableなUser Storyの切り出し方
  - ユーザー価値の高い機能を提供するUser Storyを切り出す（例: メーラー開発のUser Storyから、メール受信のUser Storyを切り出す）
  - 全体像を描くUser Storyを切り出す（例: TODOリスト一覧機能のUser Storyから、ダミーのTODOリスト表示のUser Storyを切り出す）
- 巨大なPBIの切り出し方
  - 直近取り組む分だけ小さいUser Storyとして切り出し、readyにする
  - 元のPBIがまだ大きければ、無理に全部切り出さずrefiningのまま残す（後のRefinementで再度切り出す）

**分割しすぎのアンチパターン:**

以下のようなPBIは前後のPBIと統合すべき:
- 依存ライブラリの追加のみ（それを使う機能と統合）
- インターフェース/型定義のみ（実装と統合）
- テストのみ（実装と統合 - TDDでは同一サブタスク内で完結）
- リファクタリング準備のみ（リファクタリング本体と統合）

判定基準: **そのPBI単体でbenefitを実感できるか？**

- ❌「HTTP通信ができるようになる」（準備が整っただけ）
- ✅「外部APIから天気情報を取得して表示できる」（価値が届いている）
...
`scrum.ts`の完全なテンプレート。`deno run scrum.ts | jq`でデータ抽出可能。
...
- 改善アクションを1-2個選択
- timingを決定:
  - `immediate`: Retrospective内で即座に実施
  - `sprint`: 次スプリントのサブタスクとして実施（プロセス改善）
  - `product`: 新PBIとしてProduct Backlogに追加（機能追加）
- 既存のactiveアクションをcompleted/abandonedに更新

### 5. Close
- `timing: immediate`のアクションをすべて実施し、status = "completed"に更新
- `timing: sprint`のアクションを次スプリントのサブタスクに追加
- `timing: product`のアクションをProduct Backlogに新PBIとして追加
- ダッシュボードを更新
```
...
| カテゴリ | 例 | timing |
|----------|-----|--------|
| プロンプト調整 | サブエージェント定義、スラッシュコマンド、スキル、CLAUDE.mdの更新 | immediate |
| プロセス調整 | scrum.tsへのメモ追加、DoD更新、PBIテンプレート改善 | immediate |
| ツール/環境 | MCPサーバー追加、Claude Codeフック設定、pre-commitフック | immediate |
| ドキュメント | CLAUDE.md追記、ADR作成、コード規約文書化 | sprint |
| コード品質 | テストヘルパー追加、共通ユーティリティ作成、エラーメッセージ改善 | sprint/product |
| 自動化 | 反復作業のスラッシュコマンド化、CI/CD改善 | sprint/product |

**`timing: immediate`の制約:**
- プロダクションコードの変更を伴わないこと（設定ファイル、ドキュメント、プロンプトのみ）
- 1コミットでロールバック可能な単一の論理的変更であること
